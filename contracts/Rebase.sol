...
contract Rebaser is Ownable {
    ...
    function initialize( address stakingContract_, address stakedToken_, uint256 stakedTokenId_ ) external returns ( bool ) {
        require( msg.sender == initializer );
        require( stakingContract_ != address(0) );
        stakingContract = stakingContract_;
        stakedToken = IERC1155(stakedToken_);
        stakedTokenId = stakedTokenId_;
    ...
    function circulatingSupply() public view returns ( uint ) {
        return ((stakedToken.balanceOf(stakingContract, stakedTokenId) * index()));
    }
